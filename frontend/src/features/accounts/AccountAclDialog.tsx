...
export function AccountAclDialog({ open, onOpenChange, account }: Props) {
  ...
  const results = useQueries({
    queries: [
      ...
      {
        queryKey: ['account-acls', accountId],
        queryFn: () => accountsApi.listAcls(accountId as number),
        enabled: open && typeof accountId === 'number',
      },
    ],
  })
  ...
  const eligibleUsers = useMemo(() => {
    if (!users) return []
    return users
      .filter((u) => u.role !== 'admin' && u.is_active && u.id !== currentUser?.id)
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return accountsApi.setAcls(accountId, payload)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['account-acls', accountId] })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl border-border/60 bg-card backdrop-blur-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Defina quem pode visualizar ou editar transações de{' '}
